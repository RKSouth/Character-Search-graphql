const { ApolloServer, gql } = require('apollo-server-express');
const bodyParser = require('body-parser');
const cors = require('cors');
const express = require('express');
const expressJwt = require('express-jwt');
const jwt = require('jsonwebtoken');
const db = require('./db');

// importing the built in fs module that is a built in node js api
const fs = require('fs');

const port = 9000;
const jwtSecret = Buffer.from('Zn8Q5tyZ/G1MHltc4F/gTkVJMlrbKiZt', 'base64');

const app = express();
app.use(cors(), bodyParser.json(), expressJwt({
  secret: jwtSecret,
  credentialsRequired: false
}));

// this is how we load the schema, note the file needs to be set with encoding utf8 so it's read as a string and not a binary
const typeDefs = gql(fs.readFileSync('./schema.graphql', {encoding: 'utf8'}));

const resolvers = require('./resolvers')
// creating a new apollo server instance, passing a configuration object with our type defs and resolvers
const apolloServer = new ApolloServer({ typeDefs, resolvers});
// plugging our apollo server into our existing express application
// optionally we can set a path that is where we want to expose the graphql endpoint on our server
apolloServer.applyMiddleware({app, path: '/graphql'  })


app.post('/login', (req, res) => {
  const {email, password} = req.body;
  const user = db.users.list().find((user) => user.email === email);
  if (!(user && user.password === password)) {
    res.sendStatus(401);
    return;
  }
  const token = jwt.sign({sub: user.id}, jwtSecret);
  res.send({token});
});

app.listen(port, () => console.info(`Server started on port ${port}`));
